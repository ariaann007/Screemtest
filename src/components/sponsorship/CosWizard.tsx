...
interface CosWizardProps {
  onComplete: () => void;
  initialPath?: "client" | "assisted";
}

export default function CosWizard({ onComplete, initialPath }: CosWizardProps) {
  const { currentUser } = useApp();
  const [step, setStep] = useState(initialPath ? 1 : 0);
  const [path, setPath] = useState<"client" | "assisted" | null>(initialPath || null);
  ...
  const NavButtons = ({ canNext = true }: { canNext?: boolean }) => (
    <div className="flex items-center justify-between pt-4 border-t mt-6">
      <Button variant="outline" onClick={() => (step > (initialPath ? 1 : 0)) ? setStep(s => s - 1) : onComplete()}>
        <ChevronLeft className="h-4 w-4 mr-1" /> {(step === (initialPath ? 1 : 0)) ? "Cancel" : "Back"}
      </Button>
      ...
      <div className="max-w-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            ...
            <h3 className="font-semibold mb-1">Option 2: Screem Prepares</h3>
            <p className="text-sm text-muted-foreground">Provide minimal info and upload the offer letter. Screem prepares the full draft.</p>
  ...
  if (path === "assisted") {
    ...
    // step 2 = invoice
    if (step === 2) {
      // return invoice block handled below
    }
  }
  ...
  if (step === 4) {
    ...
    const isManager = currentUser?.role === "screem_manager" || currentUser?.role === "super_admin";
